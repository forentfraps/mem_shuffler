...
pub const Shuffler = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !Self {
        ...
        return .{
            ...
            .arenas = try std.ArrayList(MemArena).initCapacity(allocator, 1),
            .active_arena = 0,
            .next_handle = 0,
    ...
    fn makeArena(self: *Self) !void {
        // caller holds lock
        const slot = try self.arenas.addOne(self.parent_allocator);
    ...
    fn shuffleUnsafe(self: *Self) !void {
        ...
        var unlocked = try std.ArrayList(Handle).initCapacity(self.parent_allocator, 1);
        defer unlocked.deinit(self.parent_allocator);

        var to_clear = try std.ArrayList(Handle).initCapacity(self.parent_allocator, 1);
        defer to_clear.deinit(self.parent_allocator);

        var arenas_to_reset = try std.ArrayList(usize).initCapacity(self.parent_allocator, 1);
        defer arenas_to_reset.deinit(self.parent_allocator);
        ...
        for (self.arenas.items, 0..) |*arena, idx| {
            ...
            while (iter.next()) |handle_ptr| {
                ...
                if (!entry.locked) {
                    ...
                    if (entry.to_clear) {
                        try to_clear.append(self.parent_allocator, h);
                    } else {
                        try unlocked.append(self.parent_allocator, h);
                    }
                }
            }
            ...
            if (arena.entries.count() == 0) {
                arena.empty = true;
                try arenas_to_reset.append(self.parent_allocator, idx);
            } else {
                arena.empty = false;
            }
        }
    ...
    pub fn rotateKey(self: *Self, new_key_opt: ?*const [32]u8, new_salt_opt: ?*const [8]u8) !void {
        ...
        var to_reencrypt = try std.ArrayList(Handle).initCapacity(self.parent_allocator, 1);
        defer to_reencrypt.deinit(self.parent_allocator);
        ...
        while (it2.next()) |kv| {
            ...
            if (entry_ptr.encrypted) {
                ...
                try to_reencrypt.append(self.parent_allocator, h);
            }
        }
